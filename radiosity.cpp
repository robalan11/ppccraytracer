...
void Radiosity::ComputeFormFactors() {
    ...
    for (int i = 0; i < num_faces; i++) {
        for (int j = 0; j < num_faces; j++) {
            formfactors[i*num_faces+j] = -1;
        }
    }
}

double Radiosity::ComputeFF(int i, int j) {
    double ff;
    if (i == j) { ff = 0; }
    else if ((mesh->getFace(i)->computeNormal() - mesh->getFace(j)->computeNormal()).Length() < 0.01) { ff = 0; }
    else {
        Vec3f center = mesh->getFace(i)->computeCentroid();
        Vec3f norm = mesh->getFace(i)->computeNormal();
        Vec3f targetcenter = mesh->getFace(j)->computeCentroid();
        
        Vec3f i_to_j = targetcenter-center;
        double dist_i_to_j = i_to_j.Length();
        i_to_j.Normalize();
        
        if (norm.Dot3(i_to_j) < 0) {
            ff = 0;
        } else {
            bool blocked = false;
            if (args->num_shadow_samples > 0) {
                Ray blockingray(center, i_to_j);
                Hit blockinghit;
                raytracer->CastRay(blockingray, blockinghit, false);
                if (blockinghit.getT() < (dist_i_to_j - 0.001)) { blocked = true; }
            }
            if (blocked) {
                ff = 0;
            } else {
                Vec3f p1 = ((*(mesh->getFace(j)))[0])->get() - center; p1.Normalize();
                Vec3f p2 = ((*(mesh->getFace(j)))[1])->get() - center; p2.Normalize();
                Vec3f p3 = ((*(mesh->getFace(j)))[2])->get() - center; p3.Normalize();
                Vec3f p4 = ((*(mesh->getFace(j)))[3])->get() - center; p4.Normalize();
                
                p1 = Vec3f(p1.x()*(1-fabs(norm.x())), p1.y()*(1-fabs(norm.y())), p1.z()*(1-fabs(norm.z())));
                p2 = Vec3f(p2.x()*(1-fabs(norm.x())), p2.y()*(1-fabs(norm.y())), p2.z()*(1-fabs(norm.z())));
                p3 = Vec3f(p3.x()*(1-fabs(norm.x())), p3.y()*(1-fabs(norm.y())), p3.z()*(1-fabs(norm.z())));
                p4 = Vec3f(p4.x()*(1-fabs(norm.x())), p4.y()*(1-fabs(norm.y())), p4.z()*(1-fabs(norm.z())));
                
                double len12 = (p2-p1).Length();
                double len23 = (p2-p3).Length();
                double len34 = (p4-p3).Length();
                double len41 = (p4-p1).Length();
                double angle2 = acos((p1-p2).Dot3(p3-p2) / (len12 * len23));
                double angle4 = acos((p3-p4).Dot3(p1-p4) / (len34 * len41));
                
                double area = len12*len23*sin(angle2) + len34*len41*sin(angle4);
                
                ff = area / (2*3.14159265);
            }
        }
    }
    return ff;
...
double Radiosity::Iterate() {
    ...
    if (formfactors[max_undistributed_patch*num_faces+max_undistributed_patch] == -1) {
        for (int i = 0; i < num_faces; i++) {
            formfactors[max_undistributed_patch*num_faces+i] = ComputeFF(max_undistributed_patch, i);
        }
    }
    
    for (int i = 0; i < num_faces; i++) {
        Material *mat = mesh->getFace(i)->getMaterial();
        Vec3f dist = undistributed[max_undistributed_patch]*formfactors[max_undistributed_patch*num_faces+i]*sqrt(area[max_undistributed_patch]/area[i]);
        absorbed[i] += dist;
        undistributed[i] += dist*mat->getDiffuseColor();
        absorbed[i] -= dist*mat->getDiffuseColor();
        radiance[i] += dist*mat->getDiffuseColor();
    }
    
    undistributed[max_undistributed_patch] = Vec3f(0,0,0);
